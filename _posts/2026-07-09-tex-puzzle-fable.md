...
Fable 5 with Claude code CLI. Instruction was minimal: look at the `rubikcube` package and make a similar package for other puzzles. I didn't asked it to work on all the puzzles at once, but one by one.
I didn't write any single line of code.

The most surprising puzzle was Square-1 and Dogic. [Square-1](https://en.wikipedia.org/wiki/Square-1_(puzzle)) is a puzzle where there are invalid moves depending on the state of the puzzle. Also, it's shape is not fixed. Fable decided to draw it by drawing top, middle, and bottom layers separately. 
...
This is a puzzle that discontinued from 2010, and now it is very hard to find.[^1]
In particular, there's no standard notations for Dogic, so I asked Fable to make up a notation.
...
Do I *really* need Fable 5 to do this? I don't know, since I haven't tried with Claude Opus 4.8 or Codex 5.5. Only thing I know is that Fable 5 did the job.
...
Although I didn't fully understand the code, it seems that verification of large puzzles uses [Schrier-Sims algorithm](https://en.wikipedia.org/wiki/Schreier%E2%80%93Sims_algorithm).
...
[^1]: You can find this on eBay probably once in a year. I have one, which I bought about 10 years ago.